...
def getitem(idx, data, batches, start_id, src, is_add_cls):
    ...
    # index start at 0
    if src: # littlemilk: for exposure
        if is_add_cls is True: # littlemilk: for non-transformer decoder
            batch = [[2] + data[i - start_id] for i in sequence_indices]
        else: # littlemilk: for transformer decoder
            batch = [data[i - start_id] for i in sequence_indices]
    else: # littlemilk: for behavior
