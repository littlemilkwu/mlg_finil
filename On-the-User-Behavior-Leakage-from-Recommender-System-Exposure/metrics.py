...
def metric_at_k_set(actual, predicted, topk, tgt_len):
    ...
    for i in range(num_users):
        ...
        pred_list = predicted[i]
        pred_set = set()
        if len(pred_list) != tgt_len or len(act_set) != tgt_len: # littlemilk: comment
            continue
        total_user += 1
    ...
    MRR /= tgt_len
    NDCG /= tgt_len
    if total_user == 0 or total_user == 0.0:
        print('metrics: total_user equal 0, division by zero error')
        return 0, 0, 0
    else:
        return NDCG / total_user, MRR / total_user, Recall / total_user
