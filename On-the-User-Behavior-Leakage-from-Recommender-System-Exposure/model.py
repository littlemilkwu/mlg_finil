...
class PoolEncoder(nn.Module):
    ...
    def forward(self, src):
        ...
        enc_states = pool
        if self.is_add_cls: # littlemilk: for non-transformer decoder, shape=(266,128)
            return enc_states
        else:   # littlemilk: for transformer decoder, shape=(10, 266, 128)
...
class RNNDecoder(nn.Module):
    ...
    def forward(self, dec_input, enc_state, state=None, cell=None):
        dec_input = self.emc(dec_input) # littlemilk: (6, 266) -> (6, 266, 128)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src, src_key_padding_mask=None):
        ...
        if self.is_add_cls:
            ...
            return memory[:, 0, :]  # littlemilk: 非 transforder decoder (266, 11, 128)
        else:
            return memory   # littlemilk: transforder decoder (10, 266, 128)
...
class PointDecoder(nn.Module):
    def __init__(self, tgt_vocab_size, d_model):
        ...
        super().__init__()
        self.d_model = d_model
        self.embed_tgt = nn.Embedding(tgt_vocab_size, d_model) # littlemilk: where using this?
        self.fc = nn.Linear(d_model, tgt_vocab_size)

    def forward(self, enc_states):
        # Run the output through a fc layer to return values for each token in the vocab
        fc = self.fc(enc_states)    # littlemilk: (266, 128) -> (266, 64577)
        return fc   
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, tgt_key_padding_mask, memory_key_padding_mask, tgt_mask, enc_states):
        ...
        tgt = self.pos_enc(self.embed_tgt(tgt) * math.sqrt(self.d_model))   # littlemilk: (6, 266) -> (6, 266, 128) -> (6, 266, 128)
        print('he', enc_states.shape)
        print('she', tgt.shape, tgt)
        exit()
        output = self.decoder.forward(tgt, memory=enc_states, tgt_mask=tgt_mask, tgt_key_padding_mask=tgt_key_padding_mask,
